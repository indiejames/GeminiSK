...
require 'optparse'
 ...
 FileUtils.cp("#{options[:source_dir]}/GeminiSK-Prefix.pch", "#{outpath}/Prefix.pch")
 FileUtils.cp("#{options[:template_dir]}/TemplateInfo.plist.geminisk",  "#{outpath}/TemplateInfo.plist")
 ...
 Dir.entries(options[:source_dir]).each do |path|
   if !FileTest.directory?(path) && path =~ /.*(m|mm|h|fsh|vsh|icns|lua|png)$/
